...
# -----------------------------
max_levels 		   = 30 		# Max levels in hierarchy
...
# interp_smooth = ('gauss_seidel', {'sweep': 'symmetric', 'iterations': 1} )
interp_smooth = ('richardson', {'omega': 1} )
...
relaxation = ('richardson', {'omega':3.0/2.0, 'iterations': 1})


# Adaptive parameters
# -------------------
candidate_iters		= 30 	# number of smoothings/cycles used at each stage of adaptive process
num_candidates 		= 1		# number of near null space candidated to generate
improvement_iters 	= 0		# number of times a target bad guy is improved
...
# improve_candidates = [('jacobi', {'omega': 3.0/3.0, 'iterations': 4}), None]
...
sa_eff_conv = (sa_residuals[-1]/sa_residuals[0]) ** (1/(ml_sa.cycle_complexity()*len(sa_residuals)))

print "Smoothed aggregation - ", sa_time, " seconds"
print ml_sa
print sa_conv_factors
print "Effective convergence", sa_eff_conv
print
...
start = time.clock()
[ml_asa, work] = adaptive_sa_solver(A, initial_candidates=bad_guy, pdef=is_pdef,
                                    num_candidates=num_candidates,
									candidate_iters=candidate_iters,
                                    improvement_iters=improvement_iters,
									epsilon=target_convergence, max_levels=max_levels,
                                    max_coarse=max_coarse, aggregate=aggregation,
                                    prepostsmoother=relaxation, smooth=interp_smooth,
...
asa_eff_conv = (asa_residuals[-1]/asa_residuals[0]) ** (1/(ml_asa.cycle_complexity()*len(asa_residuals)))

print "Classical aSA - ", asa_time, " seconds"
print ml_asa
print asa_conv_factors
print "Effective convergence", asa_eff_conv
print
...
[ml_new_asa, work] = asa_solver(A, initial_targets=bad_guy,
			                    max_targets=max_targets,
	                            min_targets=min_targets,
	                            num_initial_targets=num_candidates,
	                            ...
	                            smooth=interp_smooth, strength=strength_connection,
                                aggregate=aggregation,
	                            coarse_solver=coarse_solver, coarse_size=max_coarse, verbose=False)
...
new_asa_eff_conv = (new_asa_residuals[-1]/new_asa_residuals[0]) ** (1/(ml_new_asa.cycle_complexity()*len(new_asa_residuals)))

print "New  aSA - ", new_asa_time, " seconds"
print new_asa_conv_factors
print "Effective convergence", new_asa_eff_conv
print

# pdb.set_trace()

# print "%s,%.4f,%.4f,%d,%d,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f"%(relaxation[0],weak_tol,local_weak_tol,N,len(ml_new_asa.levels),max_level_iterations,new_asa_conv_factors[-1],asa_conv_factors[-1],sa_conv_factors[-1], new_asa_eff_conv, asa_eff_conv, sa_eff_conv)
