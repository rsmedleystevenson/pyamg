...
/* Bad news TODO */
//	--> This should be formulated using Acf^T, not Afc.
//		Make it general for non-symmetric now, not later. 
                             ...
                             const T filter_theta = 0)
{
	...
	// as represented in the splitting array.
		...
		// minimization and multiply by A_{cc} to get row of P. 
			...
			for (int j=0; j<Cpts_size; j++) {
				...
				for (int k=Acc_colptr[j]; k<Acc_colptr[j+1]; k++) {
					for (int i=temp_v0; i<submat_n; i++) {
						// Can break here because indices are sorted increasing
						if ( col_inds[i] > Acc_rowinds[k] ) {
							break;
						}
						// If nonzero, add to dot product 
						else if (col_inds[i] == Acc_rowinds[k]) {
