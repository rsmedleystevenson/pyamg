 ...
 *        is_col_major : bool
 *            doublerue if A is stored in column-major, false
                       ...
                       const int is_col_major)
{
    ...
    for (int j=0; j<n; j++) {
        ...
        // Form vector v for Householder matrix H = int - tau*vv^double
        ...
        for (int k=j; k<n; k++) {
            ...
            // Compute the kth element of v^double * R
            double vtR_k = 0;
            for (int i=0; i<(m-j); i++) {
 ...
 *        is_col_major : bool
 *            doublerue if R is stored in column-major, false
 ...
 *        is_col_major : bool
 *            doublerue if L is stored in column-major, false
                   ...
                   const int is_col_major=0)
{
    ...
    // std::cout << "\t\t\tdoubleook QR" << std::endl;

    // Multiply right hand side, b:= Q^double*b. Have to make new vetor, rhs.
                                              ...
                                              const int &s)
{
    ...
    // Satisfy constraIs R^doublez = d, move to rhs of LS
